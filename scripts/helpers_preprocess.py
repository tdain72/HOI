...
ANN_FILE_train = all_data_dir \
    + 'Annotations_vcoco/train_annotations.json'
...
def get_detections(segment_key, flag):
    ...
    if len(d_o_boxes) > select_threshold - 1:
        (d_o_boxes, scores_objects, class_id_objects) = \
            (d_o_boxes[0:select_threshold - 1],
             scores_objects[0:select_threshold - 1],
             class_id_objects[0:select_threshold - 1])
...
def get_compact_label(segment_key, flag):
    ...
    no_person_dets = len(d_p_boxes)
    no_object_dets = len(d_o_boxes)
    labels_np = np.zeros([no_person_dets, no_object_dets + 1,
                         NO_VERBS], np.int32)
    ...
    if no_person_dets != 0 and len(a_p_boxes) != 0:
        ...
        for dd in range(no_person_dets):
            ...
            noobject_hois = [oi for oi in hoi_anns if len(oi['obj_box'
                             ]) == 0]
...
def clean_up_annotation(annotation):
    ...
    for hoi in annotation:
        ...
        if len(objects['obj_bbx']) == 0:  # no obj case
            cur_oi = {'verb': hoi['Verbs'], 'obj_box': []}
        else:

                        # 'obj_str': '',

            cur_oi = {'verb': hoi['Verbs'], 'obj_box': [int(coord)
                      for coord in hoi['object']['obj_bbx']]}
...
def get_boxes_det(dets, img_H, img_W):
    ...
    for det in dets:
        ...
        (left, top, right, bottom) = (left * img_W, top * img_H, right
                * img_W, bottom * img_H)
...
def dry_run():
    ALL_SEGS_train = list(ANNOTATIONS_train.keys())
    ALL_SEGS_val = list(ANNOTATIONS_val.keys())
    ALL_SEGS_test = list(ANNOTATIONS_test.keys())
    ...
    return (bad_detections_train, bad_detections_val,
            bad_detections_test)
...
if __name__ == '__main__':
    ...
    for flag in phases:
        if flag == 'train':
            ...
            ALL_SEGS = list(ANNOTATIONS_val.keys())
