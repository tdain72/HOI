...
from dataloader_vcoco import Rescale, ToTensor, vcoco_Dataset, \
    vcoco_collate
...
dataloader_train = DataLoader(  # num_workers=batch_size
    vcoco_train,
    batch_size,
    shuffle=True,
    collate_fn=vcoco_collate,
    num_workers=4,
    worker_init_fn=_init_fn,
    )
dataloader_val = DataLoader(  # num_workers=batch_size
    vcoco_val,
    batch_size,
    shuffle=True,
    collate_fn=vcoco_collate,
...
dataloader_test = DataLoader(  # num_workers=batch_size
    vcoco_test,
    batch_size,
    shuffle=False,
    collate_fn=vcoco_collate,
...
optim1 = optim.SGD([{'params': trainables, 'lr': learning_rate},
                   {'params': spmap, 'lr': 0.001}], momentum=0.9,
                   weight_decay=0.0001)
lambda1 = lambda epoch: (1.0 if epoch < 10 else (10 if epoch
                         < 28 else 1))
...
if resume_model == 't':
    try:
        ...
        print('=> loaded checkpoint when best_prediction {} and epoch {}'.format(mean_best,
              checkpoint['epoch']))
    except:
