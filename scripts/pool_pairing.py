...
def extract_spatial(hum_box, obj_box):
    ...
    if wo != 0 and ho != 0:
        extract = torch.FloatTensor([diff_x / wo, diff_y / ho,
                                    math.log(wh / wo), math.log(hh
                                    / ho)])
    elif wo == 0 and ho != 0:
        extract = torch.FloatTensor([diff_x, diff_y / ho, math.log(wh),
                                    math.log(hh / ho)])
    elif wo != 0 and ho == 0:
        extract = torch.FloatTensor([diff_x / wo, diff_y, math.log(wh
                                    / wo), math.log(hh)])
    else:
        extract = torch.FloatTensor([diff_x, diff_y, math.log(wh),
                                    math.log(hh)])
    return extract.cuda()
...
def pairing(
    ...
    for batch in range(len(pairs_info)):
        ...
        this_batch_len = int(pairs_info[batch][0]
                             + pairs_info[batch][1])
        (batch_pers, batch_objs) = (pers[start_p:start_p
                                    + this_batch_per],
                                    objs[start_o:start_o
                                    + this_batch_obj])
        batch_context = context[batch]
        sp_locs_batch = spatial_locs[start:start + this_batch_len]
        (sp_locs_pers_batch, sp_locs_objs_batch) = \
            (sp_locs_batch[0:this_batch_per],
             sp_locs_batch[this_batch_per:this_batch_per
             + this_batch_obj])
        pers_objs = []
        for (ind_p, i) in enumerate(batch_pers):
            ...
            for (ind_o, j) in enumerate(batch_objs):
                sp_features = \
                    extract_spatial(sp_locs_pers_batch[ind_p],
                                    sp_locs_objs_batch[ind_o])
        ...
        pairs_out.append(torch.cat([pers_objs_batch,
                         batch_context.repeat(pers_objs_batch.size()[0],
                         1)], 1))
