            ...
            for (index, k) in enumerate(role_ids):
                ...
                for j in range(instances):
                    if (proper_keys[dict_index + j])[-5:] == 'agent':
                        ...
                        if agent_key in list(infer_dict.keys()):
                            if k > infer_dict[agent_key]:

                                infer_dict[agent_key] = \
                                    float(person_action_score)
                        else:
                        ...
                        infer_dict[proper_keys[dict_index + j]] = \
                            obj_bbx_score.tolist()
