...
ANN_FILE_train = all_data_dir \
    + 'Annotations_hico/train_annotations.json'
...
obj_list = dict([(value, int(key) - 1) for (key, value) in
                list(list_obj.items())])
...
def get_detections(segment_key, flag):
    if flag == 'train':
        ...
        cur_obj_path_s = OBJ_PATH_train_s + 'HICO_train2015_%.8i.json' \
            % segment_key
        SCORE_TH = 0.6
        SCORE_OBJ = 0.3
        ...
        cur_obj_path_s = OBJ_PATH_test_s + 'HICO_test2015_%.8i.json' \
            % segment_key
        SCORE_TH = 0.6
        SCORE_OBJ = 0.3
        select_threshold = 15
    ...
    img_W = detections['W']
    shape = [img_W, img_H]
    (persons_d, objects_d) = analyze_detections(detections, SCORE_TH,
            SCORE_OBJ)
    (d_p_boxes, scores_persons, class_id_humans) = \
        get_boxes_det(persons_d, img_H, img_W)
    (d_o_boxes, scores_objects, class_id_objects) = \
        get_boxes_det(objects_d, img_H, img_W)
    ...
    try:
        ...
        import pdb
        pdb.set_trace()
    class_id_objects = np.concatenate([class_id_objects,
            class_id_humans]).tolist()
    scores_objects = np.concatenate([scores_objects,
                                    scores_persons]).tolist()

    if len(d_p_boxes) > select_threshold:
        (d_p_boxes, scores_persons, class_id_humans) = \
            (d_p_boxes[0:select_threshold],
             scores_persons[0:select_threshold],
             class_id_humans[0:select_threshold])
    if len(d_o_boxes) > select_threshold:
        (d_o_boxes, scores_objects, class_id_objects) = \
            (d_o_boxes[0:select_threshold],
             scores_objects[0:select_threshold],
             class_id_objects[0:select_threshold])
...
def get_compact_detections(segment_key, flag):
    ...
    return {
        ...
        'person_bbx_score': scores_persons,
        'objects_bbx_score': scores_objects,
...
def get_compact_label(segment_key, flag):
    ...
    no_person_dets = len(d_p_boxes)
    no_object_dets = len(d_o_boxes)
    labels_np = np.zeros([no_person_dets, no_object_dets, NO_VERBS],
                         np.int32)
    ...
    if no_person_dets != 0 and len(a_p_boxes) != 0:
        ...
        for dd in range(no_person_dets):
            ...
            object_hois = [oi for oi in hoi_anns if len(oi['obj_box'])
                           != 0]
        ...
        comp_labels = labels_np.reshape(no_person_dets
                * no_object_dets, NO_VERBS)
        labels_single = np.array([(1 if i.any() == True else 0)
                                 for i in comp_labels])
        labels_single = \
            labels_single.reshape(np.shape(labels_single)[0], 1)
        return {'labels_all': labels_np, 'labels_single': labels_single}
    else:
        comp_labels = labels_np.reshape(no_person_dets
                * (no_object_dets + 1), NO_VERBS)
        labels_single = np.array([(1 if i.any() == True else 0)
                                 for i in comp_labels])
        labels_single = \
            labels_single.reshape(np.shape(labels_single)[0], 1)
...
def clean_up_annotation(annotation):
    ...
    # import pdb;pdb.set_trace()
        ...
        if len(objects['obj_bbx']) == 0:  # no obj case
            ...
            cur_obj = clean_object(object_list, np.asarray(hoi['object'
                                   ]['obj_bbx']))
            cur_oi = {  # 'obj_box':[int(coord) for coord in hoi['object']['obj_bbx']],
                'verb': hoi['Verbs'],
                'obj_box': cur_obj,
...
def get_boxes_det(dets, img_H, img_W):
    ...
    for det in dets:
        ...
        if len(det['class_str'].split()) == 2:
            ...
            str = det['class_str'].split()[0] + '_' + det['class_str'
                    ].split()[1]
        else:
            str = det['class_str']
...
def IoU_box(box1, box2):
    ...
    areaIntersection = max(0, right_int - left_int) * max(0, bottom_int
            - top_int)
...
def dry_run():
    ...
    with open(all_data_dir
              + 'bad_detections_hico/bad_detections_train.json') as fp:
    ...
    with open(all_data_dir
              + 'bad_detections_hico/bad_detections_test.json') as fp:
...
if __name__ == '__main__':
    ...
    (b_d_tr, b_d_test) = ([int(l) for l in bad_detections_train],
                          [int(l) for l in bad_detections_test])
    phases = ['train', 'test']
    ...
    for flag in phases:
        if flag == 'train':
            ALL_SEGS = list(ANNOTATIONS_train.keys())
        elif flag == 'test':

            ALL_SEGS = list(ANNOTATIONS_test.keys())
        ...
        for segkey in tqdm(ALL_SEGS):
            ...
            if segkey in b_d_tr + b_d_test:
                ...
                compact_dets[segkey] = get_compact_detections(segkey,
                        flag)
