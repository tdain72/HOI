...
class VSGNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.spmap_up = nn.Sequential(nn.Linear(32, 512), nn.ReLU())

                # ######################################

                # ## Prediction Model for attention features#######

        self.lin_spmap_tail = nn.Sequential(nn.Linear(512, 117))
        ...
        self.lin_single_head = nn.Sequential(nn.Linear(lin_size * 3
                + 4, 1024), nn.Linear(1024, 512), nn.ReLU())  # nn.Linear(2048,1),
                                                              # nn.Dropout2d(p=0.5),
                                                              # nn.Linear(lin_size*3, 1024),

        self.lin_single_tail = nn.Sequential(nn.Linear(512, 1))  # nn.ReLU(),
        ...
        self.lin_visual_head = nn.Sequential(nn.Linear(lin_size * 3
                + 4, 1024), nn.Linear(1024, 512), nn.ReLU())  # nn.Linear(2048, 117),
                                                              # nn.Dropout2d(p=0.5),
                                                              # nn.Linear(lin_size*3, 1024),
        ...
        self.lin_visual_tail = nn.Sequential(nn.Linear(512, 117))
        ...
        self.lin_graph_head = nn.Sequential(nn.Linear(lin_size * 2,
                1024), nn.Linear(1024, 512), nn.ReLU())  # nn.Linear(2048, 117),
                                                         # nn.Dropout2d(p=0.5),

        self.lin_graph_tail = nn.Sequential(nn.Linear(512, 117))
    ...
    # ### Making Essential Pairing##########
        ...
        (pairs, people, objects_only) = ROI.pairing(out2_people,
                out2_objects, out2_context, spatial_locs, pairs_info)
    ...
    # ###################################
        ...
        # ###Slicing##########....
            ...
            objects_this_batch = objects_only[start_o:start_o
                + int(l[1])][1:]
            no_objects_this_batch = objects_only[start_o:start_o
                + int(l[1])][0]
            no_obj = len(objects_this_batch)
            interaction_prob_this_batch = \
                interaction_prob[start_c:start_c + int(l[1])
                * int(l[0])]
            if no_obj == 0:

                people_this_batch_r = people_this_batch

                objects_this_batch_r = no_objects_this_batch.view([1,
                        1024])
            else:
                peo_to_obj_this_batch = torch.stack([torch.cat((i, j))
                        for (ind_p, i) in enumerate(people_this_batch)
                        for (ind_o, j) in
                        enumerate(objects_this_batch)])
                obj_to_peo_this_batch = torch.stack([torch.cat((i, j))
        ...
        # ###### Adjecency###########
                ...
                for (index_probs, probs) in \
                    enumerate(interaction_prob_this_batch):
        ...
        # ##Finding Out Refined Features######
                ...
                people_this_batch_r = people_this_batch \
                    + torch.mm(adj_po.view([no_peo, no_obj]),
                               self.peo_to_obj_w(objects_this_batch))

                objects_this_batch_r = objects_this_batch \
                    + torch.mm(adj_op.view([no_peo, no_obj]).t(),
                               self.obj_to_peo_w(people_this_batch))
                objects_this_batch_r = \
                    torch.cat((no_objects_this_batch.view([1, 1024]),
                              objects_this_batch_r))
        ...
        # ### Restructuring ####
            ...
            pairs_f.append(torch.stack([torch.cat((i, j)) for (ind_p,
                           i) in enumerate(people_t_f) for (ind_o,
                           j) in enumerate(objects_t_f)]))
