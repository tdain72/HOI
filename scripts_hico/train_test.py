...
with open(all_data_dir + 'hico_infos/mask.pickle', 'rb') as fp:
    mask_f = pickle.load(fp, encoding="latin-1")
mask_t = mask_f[0]
count_t = mask_f[1]
sigmoid = nn.Sigmoid()
...
def extend(inputt, extend_number):
    ...
    for a in inputt:
        x = np.repeat(a.reshape(1, np.shape(inputt)[-1]),
                      extend_number, axis=0)
...
def filtering(
    ...
    for (index, i) in enumerate(filters):
        ...
        res1 = np.concatenate([res1, predicted_HOI[index].reshape(1,
                              no_of_classes)], axis=0)
        res2 = np.concatenate([res2, true[index].reshape(1,
                              no_of_classes)], axis=0)
        res3 = np.concatenate([res3, predicted_HOI[index].reshape(1,
                              no_of_classes)], axis=0)
        res4 = np.concatenate([res4, persons_np[index].reshape(1, 4)],
                              axis=0)
        res5 = np.concatenate([res5, objects_np[index].reshape(1, 4)],
                              axis=0)
        res6 = np.concatenate([res6, class_ids[index].reshape(1, 1)],
                              axis=0)
   ...
   # #### Starting the Epochs#############
        ...
        # ###Starting the iterations##################
            ...
            # import pdb;pdb.set_trace()........
                ...
                # #################################
                    ...
                    index_mask = \
                        class_ids_extended[1:].reshape(len(class_ids_extended[1:])).astype('int32'
                            )
                    (loss_mask, count_weight) = (mask_t[index_mask],
                            count_t[index_mask])
            ...
            # #### Preparing for Storing Results##########....
                ...
                predicted_scores = np.concatenate((predicted_scores,
                        predicted_HOI), axis=0)
                true_scores = np.concatenate((true_scores, true),
                        axis=0)
            ...
            # ### Storing the result in V-COCO Format##########
                ...
                # prep.infer_format(image_id,all_scores,phase,detections_test,pairs_info)
                        ...
                        proper.infer_format(image_id, all_scores,
                                phase, detections_test, pairs_info)
            ...
            if phase == 'train':
                ...
                (AP, AP_single) = ap.class_AP(predicted_scores[1:, :],
                        true_scores[1:, :], predicted_scores_single[1:
                        ], true_scores_single[1:])
                AP_train = pd.DataFrame(AP, columns=['Name_TRAIN',
                        'Score_TRAIN'])
                AP_train_single = pd.DataFrame(AP_single,
                ...
                AP_test = pd.DataFrame(AP, columns=['Name_TEST',
                        'Score_TEST'])
                AP_test_single = pd.DataFrame(AP_single,
                        columns=['Name_TEST', 'Score_TEST'])
                if (epoch + 1) % saving_epoch == 0 or infr == 't':
    ...
    # ####################################
        ...
        if infr == 't':
            ...
            AP_final_single = pd.concat([AP_train_single,
                    AP_test_single], axis=1)
        ...
        try:
            print('Loss_train:{},Loss_test:{}'.format(loss_epoch_train[epoch
                    - start_epoch], loss_epoch_test[epoch
                    - start_epoch]))
        except:
            print('Loss_test:{}'.format(loss_epoch_test[epoch
                    - start_epoch]))

        print('This epoch completes in {:.0f}m {:.06f}s'.format(time_elapsed
                // 60, time_elapsed % 60))
        if infr == 't':
            break

    time_elapsed = time.time() - initial_time
    print('The whole process runs for {:.0f}h {:.0f}m {:0f}s'.format(time_elapsed
            // 3600, time_elapsed % 3600 // 60, time_elapsed % 3600
            % 60 % 60))
    return
